    ...
    public class C24PlazoFijo
    {
        private static void Genera(string sdbconexion, string sfecha, string scarpeta, string sfechac)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C24PlazoFijo.error [No se pudo establecer conexion con la base de datos]");
                }
                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prcfgcrl003varplazo";
                    ...
                    string sfile = "DatosCooperativas/" + scarpeta.Trim() + "/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "DCCaPl_" + sdbconexion.Substring(4, 2).Trim() + "_" + sfecha.Substring(0, 6) + ".inp";
                    ...
                    using (StreamWriter sw = new StreamWriter(ConfigurationManager.AppSettings["Ruta"].ToString() + sfile))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                sLinea = string.Format("{1}{0}{2}{0}{3}{0}{4:dd/MM/yyyy}{0}{5:f2}{0}{6:f2}{0}{7:f2}{0}P", "|",
                ...
                {
                    //EventLog.WriteEntry("SISCARDatosCooperativa", string.Format("C24PlazoFijo Error {0} Conexion{1} ", ex.Message, sdbconexion), //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
        ...
        private static void GeneraT24(string sdbconexion, string sfecha)
        {
            using (SqlConnection Oconexion = new SqlConnection(ConfigurationManager.ConnectionStrings["ConexionT24"].ConnectionString.ToString()))
            {
                ...
                try
                {
                    string query = "[dbo].[PROC_ARCH_DCCAPL_NEW]";
                    string select = $"select coope, COUNT(1) as conteo, sum(debitos) as debitos, SUM(creditos) as creditos, SUM(saldo) as saldo from analyticsImport.dbo.t_archivo_DCCAPL where periodo = {sfecha.Substring(0,6)} and   coope = {sdbconexion.Substring(4, 2)} group by coope";
                    int Correlativo = 0;
                    decimal SaldoFinal = 0;
