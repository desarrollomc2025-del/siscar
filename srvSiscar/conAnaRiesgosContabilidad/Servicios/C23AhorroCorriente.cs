...
{
    public class C23AhorroCorriente
    {
        private static void Genera(string sdbconexion, string sfecha, string scarpeta, string sfechac)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                try
                {
                    ...
                    string sfile = "DatosCooperativas/" + scarpeta.Trim() + "/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "DCCaCo_" + sdbconexion.Substring(4, 2).Trim() + "_" + sfecha.Substring(0, 6) + ".inp";
        ...
        private static void GeneraT24(string sdbconexion, string sfecha)
        {
            using (SqlConnection Oconexion = new SqlConnection(ConfigurationManager.ConnectionStrings["ConexionT24"].ConnectionString.ToString()))
            {
                ...
                try
                {
                    ...
                    string select = $"select coope, COUNT(1) as conteo, sum(debitos) as debitos, SUM(creditos) as creditos, SUM(saldo) as saldo from analyticsImport.dbo.t_archivo_DCCACO where periodo = {sfecha.Substring(0,6)} and   coope = {sdbconexion.Substring(4, 2)} group by coope";
                    int Correlativo = 0;
                    decimal SaldoFinal = 0;
