...
{
    public class C14ColInstrumentos
    {
        private static void GeneraT24(string sdbconexion, string sfecha, string scarpeta)
        {
            using (SqlConnection Oconexion = new SqlConnection(ConfigurationManager.ConnectionStrings["ConexionAnalytics"].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C14InstrumentoColocaciones.error [No se pudo establecer conexion con la base de datos]");
                }
                try
                {
                    string query = "[AnalyticsImport].[dbo].[PROC_ARCH_DCINCO_NEW]";
                ...
                {
                    //EventLog.WriteEntry("SISCARDatosCooperativa", string.Format("C14InstrumentoColocaciones Error {0} Conexion{1} ", ex.Message, sdbconexion), //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
        ...
        private static void Genera(string sdbconexion, string sfecha, string scarpeta)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C14ColInstrumentos.error [No se pudo establecer conexion con la base de datos]");
                }

                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prccolinstrument";
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("CURSOR_", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                    string sfile = "DatosCooperativas/" + scarpeta.Trim() + "/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "DCInCo_" + sdbconexion.Substring(4, 2).Trim() + "_" + sfecha.Substring(0, 6) + ".inp";
                    ...
                    using (StreamWriter sw = new StreamWriter(ConfigurationManager.AppSettings["Ruta"].ToString() + sfile))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                sLinea = reader["colnuminstrum"].ToString().Trim() + "|" +
                                            sdbconexion.Substring(4, 2).Trim() + "|" +
                                            reader["coldesinstrum"].ToString().Trim() + "|" +
                                            reader["colcodmoneda"].ToString().Trim();
                                sw.WriteLine(sLinea);
                            }
                        }
                    }
                ...
                {
                    //EventLog.WriteEntry("SISCARDatosCooperativa", string.Format("C14ColInstrumentos Error {0} Conexion{1} ", ex.Message, sdbconexion), //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
