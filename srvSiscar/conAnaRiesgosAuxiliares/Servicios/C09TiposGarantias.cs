...
{
    public class C09TiposGarantias
    {
        private static void Genera(string sdbconexion, string sfecha)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C09TiposGarantias.error [No se pudo establecer conexion con la base de datos]");
                }
                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prcgartipogarantia";
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("CURSOR_", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                    string sfile = "CatalogosGenerales/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "CGGara_" + sfecha + ".inp";
                    using (StreamWriter sw = new StreamWriter(ConfigurationManager.AppSettings["Ruta"].ToString() + sfile))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                sLinea = reader["gartipogar"].ToString().Trim() + "|" +
                                         reader["gardestipogar"].ToString().Trim();
                                sw.WriteLine(sLinea);
                            }
                        }
                    }
                ...
                {
                    //EventLog.WriteEntry("SISCARCatalogos", "C09TiposGarantias Error " + ex.Message, //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
