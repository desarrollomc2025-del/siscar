    ...
    public class C11ColFrecuePag
    {
        private static void Genera(string sdbconexion, string sfecha)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C11ColFrecuePag.error [No se pudo establecer conexion con la base de datos]");
                }

                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prccolfrecuepago";
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("CURSOR_", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                    string sfile = "CatalogosGenerales/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "CGFrPa_" + sfecha + ".inp";
                    using (StreamWriter sw = new StreamWriter(ConfigurationManager.AppSettings["Ruta"].ToString() + sfile))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                sLinea = reader["colcodfrecpag"].ToString().Trim() + "|" +
                                            reader["coldesfrecpag"].ToString().Trim();
                                sw.WriteLine(sLinea);
                            }
                        }
                    }
                ...
                {
                    //EventLog.WriteEntry("SISCARCatalogos", "C11ColFrecuePag Error " + ex.Message, //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
