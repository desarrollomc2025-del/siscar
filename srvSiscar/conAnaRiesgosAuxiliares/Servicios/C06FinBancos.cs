    ...
    public class C06FinBancos
    {
        private static void Genera(string sdbconexion, string sfecha)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C06FinBancos.error [No se pudo establecer conexion con la base de datos]");
                }

                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prcfinbancos";
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("CURSOR_", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                    string sfile = "CatalogosGenerales/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "CGEnFi_" + sfecha + ".inp";
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                if (cmd.ExecuteNonQuery() != 0)
                                {
                                    while (reader.Read())
                                    {
                                        sLinea = reader["fincodbanco"].ToString().Trim() + "|" +
                                                 reader["findesbanco"].ToString().Trim();
                                        sw.WriteLine(sLinea);
                                    }
                                }
                            }
                ...
                {
                    //EventLog.WriteEntry("SISCARCatalogos", "C06FinBancos Error " + ex.Message, //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
