    ...
    public class C04FinDeptos
    {
        private static void Genera(string sdbconexion, string sfecha)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C04FinDeptos.error [No se pudo establecer conexion con la base de datos]");
                }

                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prcfindeptos";
                    ...
                    string sfile = "CatalogosGenerales/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "CGDepa_" + sfecha + ".inp";
                                ...
                                if (cmd.ExecuteNonQuery() != 0)
                                {
                                    while (reader.Read())
                                    {
                                        sLinea = reader["fincoddepto"].ToString().Trim() + "|" +
                                                 reader["findesdepto"].ToString().Trim();
                                        sw.WriteLine(sLinea);
                                    }
                                }
                            }
                ...
                {
                    //EventLog.WriteEntry("SISCARCatalogos", "C04FinDeptos Error " + ex.Message, //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
