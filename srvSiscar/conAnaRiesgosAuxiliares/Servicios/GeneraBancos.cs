...
{
    public class GeneraBancos
    {
        private static void Con_BorraBancos(string fechaInicial, string fechaFinal, string cooperativa)
        {
            using (SqlConnection Oconexion = new SqlConnection(ConfigurationManager.ConnectionStrings["ConexionSQL"].ConnectionString.ToString()))
            {
                ...
                Oconexion.Query("[dbo].[PROC_BANCOS_DEL]", new { codigocooperativa = $"{cooperativa.Substring(4, 2).Trim()}",FechaInicial = fechaInicial,FechaFinal = fechaFinal}, commandType: CommandType.StoredProcedure);
            }
        }
                    ...
                    using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ConexionSQL"].ConnectionString))
                    {
                        ...
                        try
                        {
                            ...
                            cmd.Parameters.Add(new SqlParameter()
                            {
                                ...
                                Value = cooperativa.Substring(4,2) 
                            });
                            cmd.Parameters.Add(new SqlParameter()
                            {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ConexionSQL"].ConnectionString))
                {
                    ...
                    try
                    {
                        ...
                        {
                            ParameterName = "@FechaFinal",
                            Value = string.Format("{0:yyyyMMdd}", fechaFinal)
        ...
        {
            Con_BorraBancos(fechaInicial, fechaFinal, cooperativa);
