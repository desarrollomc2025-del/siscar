    ...
    public class C01Entidades
    {
        private static void Genera(string sdbconexion, string sfecha)
        {
            using (OracleConnection Oconexion = new OracleConnection(ConfigurationManager.ConnectionStrings[sdbconexion].ConnectionString.ToString()))
            {
                ...
                {
                    throw new Exception("C01Entidades.error [No se pudo establecer conexion con la base de datos]");
                }

                try
                {
                    OracleCommand cmd = Oconexion.CreateCommand();
                    cmd.CommandText = "bkwanaries_pkg.prcsegentidades";
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.Add("CURSOR_", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                    string sfile = "CatalogosGenerales/" + sfecha.Substring(0, 4).Trim() + "-" + sfecha.Substring(4, 2).Trim() + "/" + "CGCoop_" + sfecha.Substring(0,6) + ".inp";
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                if (cmd.ExecuteNonQuery() != 0)
                                {
                                    while (reader.Read())
                                    {
                                        sLinea = reader["segcodentidad"].ToString().Trim() + "|" +
                                                 reader["segnombre"].ToString().Trim();
                                        sw.WriteLine(sLinea);
                                    }
                                }
                            }
                    ...
                    //ftpTraslada.upload("CatalogosGenerales/" + sfecha + "/" + sfile, ConfigurationManager.AppSettings["Ruta"].ToString() + sfile);
                ...
                {
                    //EventLog.WriteEntry("SISCARCatalogos", "C01Entidades Error " + ex.Message, //EventLogEntryType.Error, 234);
                }
            }
        }//Genera
